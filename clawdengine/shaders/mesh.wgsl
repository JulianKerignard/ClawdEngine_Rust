...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    let raw_T = (model.model * vec4<f32>(in.tangent.xyz, 0.0)).xyz;
    let N_raw = normalize((model.model * vec4<f32>(in.normal, 0.0)).xyz);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    // TBN normal mapping (guard against zero tangent → NaN)
    ...
    for (var i = 0u; i < min(lights.count, 4u); i++) {
        ...
        // Light type: position.w: 0=directional, 1=point, 2=spot
        var L: vec3<f32>;
        var attenuation = 1.0;
        if (light.position.w < 0.5) {
            ...
            // Spot light cone attenuation (position.w == 2.0)
            // direction.xyz = spot direction, direction.w = range
            // spot_params.x = cos_inner, spot_params.y = cos_outer
        ...
        // Shadow (only first directional light casts shadows)
        var shadow = 1.0;
